  ...
  if (!week) {
    return (
      <div className="p-8">
        ...
        <p className="mt-4 text-ink-soft">{t("detail.notFound")}</p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl px-5 py-6 lg:px-8">
      ...
      <header className="mt-4 card p-6">
        ...
        <p className="mt-3 inline-flex items-center gap-3 text-sm text-ink-soft">
          <span className="inline-flex items-center gap-1.5">
            <Clock size={15} /> {t("detail.total")} {formatMinutes(week.total_time_minutes)}
          </span>
          <span className="font-medium text-ink">
            {t("detail.topicsDone", { done, total: week.topics.length })}
          </span>
        </p>
      </header>
      ...
      <div className="mt-5 rounded-card bg-primary-light p-5">
        <p className="text-sm font-medium text-primary">
          🏁 {t("detail.milestone", { m: week.milestone })}
        </p>
      </div>
      ...
      >
        {status === "completed" ? (
          <>
            <CheckCircle2 size={16} /> {t("detail.completedUndo")}
          </>
        ) : (
          <>
            <Circle size={16} /> {t("detail.markAll")}
          </>
        )}
      </button>
    </div>
